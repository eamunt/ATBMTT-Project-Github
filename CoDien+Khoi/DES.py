...
# Hàm hoán vị, dò vào trong bản hoán vị
def permute(k, arr, n):
    permutation = ""
    for i in range(0, n):
...
def xor(a, b): # có thể dùng hàm của python : "^"
    ans = ""
...
# Hoán vị P (của hàm Feistel)( sau khi S-box xong)
per = [ 16, 7, 20, 21,
        29, 12, 28, 17,
        1, 15, 23, 26,
        5, 18, 31, 10,
...
# Bảng hoán vị cuối FP, sau khi FP sẽ ra pipher text
...
def encrypt(pt, rkb, rk):
    pt = hex2bin(pt) # chuyển sang chuỗi nhị phân
    ...
    for i in range(0, 16):
        ...
        # XOR RoundKey[i] và right_expanded , XOR với khóa vòng
        ...
        # Hoán vị P, đến đây  đã ra khoải hàng F
...
# Phân chia khóa thành nửa trái nửa phải của khóa.
...
for i in range(0, 16):
    ...
    left = shift_left(left, shift_table[i]) # có sẵn số bit dịch bên trên
    right = shift_left(right, shift_table[i])

    # Kết hợp nủa trái và phải
    combine_str = left + right # chuỗi bit nhị phân
